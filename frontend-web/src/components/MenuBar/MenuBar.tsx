...
import { LayoutDashboard, Receipt, User, Moon, Sun, Menu, X, LogOut, Phone, Wallet } from 'lucide-react';
import './MenuBar.css';

const MenuBar: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { phoneNumber, clearUserId } = useUser();
  ...
  const handleLogout = (): void => {
    if (window.confirm('Do you want to switch to a different phone number?')) {
      clearUserId();
      navigate('/login');
    }
  };
